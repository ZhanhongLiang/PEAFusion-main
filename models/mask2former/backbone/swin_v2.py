...
from models.modules import ClassAwareSeMoEFusionBlock, SeMoEFusionBlock
...
class Fusion_Swin_Transformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.use_semoe_fusion = kwargs.get("use_semoe_fusion", False)
    ...
    def init_weights(self):
        ...
        if isinstance(self.pretrained, str):
            ...
            loaded_names = sorted(set(model_state_dict.keys()) & set(filtered_state_dict.keys()))
            incompatible = self.load_state_dict(filtered_state_dict, strict=False)

            missing_names = sorted(incompatible.missing_keys)
            unexpected_names = sorted(incompatible.unexpected_keys)

            print("=== Pretrained Load Summary ===")
            print("Successfully loaded count: {}".format(len(loaded_names)))
            print("Missing count: {}".format(len(missing_names)))
            print("Unexpected count: {}".format(len(unexpected_names)))
    ...
    def forward(self, x1, x2):
        ...
        if self.model_output_attn:
            ...
            for i_layer in range(self.num_layers):
                ...
                out = self._fuse_stage_features(x1_, x2_, H, W, i_layer)
    ...
    def _fuse_stage_features(self, x1_, x2_, H, W, i_layer):
        if self.use_semoe_fusion and self.fusion_type == "semoe":
            rgb_feat = x1_.view(-1, H, W, self.num_features[i_layer]).permute(0, 3, 1, 2).contiguous()
            thermal_feat = x2_.view(-1, H, W, self.num_features[i_layer]).permute(0, 3, 1, 2).contiguous()
            stage_output = self.semoe_fusion_blocks[i_layer](rgb_feat, thermal_feat)
        ...
        return out.view(-1, H, W, self.num_features[i_layer]).permute(0, 3, 1, 2).contiguous()
...
class RGBTSwinTransformer(Fusion_Swin_Transformer, Backbone):
    def __init__(self, cfg, input_shape):
        ...
        use_semoe_fusion = cfg.MODEL.FUSION.USE_SEMOE_FUSION
        ...
        super().__init__(
            ...
            model_ffn_ratio,
            model_output_attn,
            use_semoe_fusion=use_semoe_fusion,
